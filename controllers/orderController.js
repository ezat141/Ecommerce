...
exports.getOrdersByRating = async (req, res) => {
    const { rating } = req.body;

    try {
        const orders = await Order.find({ orders_rating: rating }).populate('orders_addressid');

        if (orders.length === 0) {
            return res.status(404).json({ status: httpStatusText.FAIL, message: 'No orders found with the specified rating' });
        }
